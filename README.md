...
- 播放页保持固定视口高度，倍速、曝光和音量使用对齐的紧凑滑条；全景视频支持独立音量滑条、拖动、触控板双指滑动、触屏双指缩放和触控板缩放来控制视角与视野，双击全景播放器可全屏。
- 界面固定为浅色主题。
- 设置页可调整页面标题、目标路径、按小时计的扫描间隔、默认播放音量、视频扩展名、日期/大小/时长显示项；扫描间隔为 0 时关闭自动全盘扫描，默认播放音量会同步写入数据库。
